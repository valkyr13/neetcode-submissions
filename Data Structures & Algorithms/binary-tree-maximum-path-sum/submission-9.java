...
class Solution {
    public int[] maximisePathSum(TreeNode root){
        ...
        ans[0] = Math.max(root.val, Math.max(root.val+leftPath, root.val+rightPath));
...
System.out.println("root: "+root.val+", right ans: "+rightAns[0]+", right ans_1: "+rightAns[1]);
        ...
        if(root.right == null){
            ...
            ans[1] = Math.max(leftAns[1],root.val+leftPath);
            return ans;
        }

        if(root.left == null){
            ans[1] = Math.max(rightAns[1],root.val+rightPath);
            return ans;
        }

        ans[1] = Math.max(Math.max(leftAns[1], rightAns[1]),root.val+leftPath+rightPath);
